    ...
    async execute(message, args, guildConfig) {
        ...
        switch (args[0]) {
            ...
            case 'userinfo':
                ...
                break;
            default: 
                message.channel.send("No test specified! You should know better!");
                break;
        }
    }
}
...
function flg_rules(guild) {
    ...
    .addField('§1.3 Datenschutz', 'Private Daten wie Telefonnr., Adressen, Passwörter, private Bilder, etc. dürfen nicht öffentlich ausgetauscht werden, zudem dürfen Daten ohne Erlaubnis des/der Besitzers/in nicht weitergegeben werden – auch nicht privat.')
    ...
    .addField('§3.2', 'Wer in einem Stream dabei ist (#stream Sprachkanal), gibt sein Einverständnis zu Sprachaufnahmen nach §3.1 durch den Beitritt in den Kanal ab.')
    ...
    .addField('§4.2', 'Gewinner*innen müssen sich innerhalb von 24 Stunden nach Ende der Verlosung beim Ausrichter*in der Verlosung melden um den Gewinn einzufordern, ansonsten wird neu ausgelost.')
    .addField('§4.3', 'Anspruch auf einen Gewinn hat nur, wer zum Zeitpunkt der Auslosung Mitglied auf dem Server ist, **nicht** gebannt ist und das Mindestalter für den Gewinn (Spiele mit Altersbeschränkung, etc.) erfüllt.')
    .addField('§4.4', 'Die Gewinnchancen mit unfairen Mitteln zu heben, ist verboten und wird entsprechend geahnded.')
    .addField('§4.5', 'Aus Fairness-Gründen und weil wir wollen, dass jede*r in den Genuss eines Gewinns kommen kann, behalten wir uns vor eine Auslosung zu wiederholen, sollte eine Person gelost worden sein, die bereits eine der letzten 3 Verlosungen gewonnen hat.'),
    new DiscordMessageEmbed()
    .setColor('#be0027')
    .setTitle('§5 - Konsequenzen bei Regelbrüchen')
    .addField('§5.1', 'Unwissenheit schützt vor Strafe nicht!')
    .addField('§5.2', 'Wir behalten uns vor bei größeren und in kürzester Zeit häufigen Vergehen, jede*n sofort vom Discord zu verweisen.')
    .addField('§5.3', 'Wir sind nicht gewillt, Mitglieder zu dulden, die sich bewusst in den Grauzonen unseres Regelwerks bewegen.')
    .addField('§5.4', 'Jeder Regelbruch wird in der Regel zunächst mit einer Verwarnung geahndet.')
